...
export const advanceCalendarDate = (date: CalendarDate, recurrence: Recurrence, steps = 1): CalendarDate => {
  ...
  switch (recurrence.frequency) {
    ...
    case "monthly": return addAnchoredMonths(date, steps, anchor.day);
    case "quarterly": return addAnchoredMonths(date, 3 * steps, anchor.day);
    case "yearly": {
    ...
    case "custom": {
      ...
      if (unit === "month") return addAnchoredMonths(date, count * steps, anchor.day);
      const { year } = parseCalendarDate(date);
