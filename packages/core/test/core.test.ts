...
describe("validation", () => {
  it.each(["2025-02-29", "2025-13-01", "not-a-date"])(
    "rejects invalid date %s",
    (nextDueDate) => {
      expect(
        safeValidateRecurringPayment({ ...payment(), nextDueDate }).success,
      ).toBe(false);
    },
  );
  ...
  it.each([-1, 1.5, Number.MAX_SAFE_INTEGER + 1])(
    ...
    (amount) => {
      expect(
        safeValidateRecurringPayment({ ...payment(), amount }).success,
      ).toBe(false);
    },
  );
...
describe("queries", () => {
  ...
  it("searches text and filters category and status", () => {
    ...
    expect(
      filterPayments(items, { statuses: ["paused", "archived"] }).map(
        ({ id }) => id,
      ),
    ).toEqual(["paused", "archived"]);
  });
